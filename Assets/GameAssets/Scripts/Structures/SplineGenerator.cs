...
public class SplineGenerator : StructureGenerator
{
    ...
    public override void Generate(StructureMaster structureMaster)
    {
        ...
        foreach (Connection endConnection in linkedSection.GetAllConnectionList())
        {
            ...
            {
                //Random
                Vector3 endPos = -startConnection.connectionTransform.forward * Random.Range(3f, 6f) + startConnection.connectionTransform.right * Random.Range(-3f, -3f);
                endConnection.connectionTransform.position = endPos;
                endConnection.connectionTransform.rotation = Quaternion.Euler(0f, startConnection.connectionTransform.eulerAngles.y + Random.Range(-120f, 120f), 0f);
            }
    ...
    public void CreatePath(Connection start, Connection end)
    {
        ...
        GameObject mesh = Spline.active.GenerateMeshAlongPath(meshPath, splineMeshInfoCopy);
